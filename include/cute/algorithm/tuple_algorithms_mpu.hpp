...
#include <cute/container/tuple_mpu.hpp>
#include <cute/algorithm/functional_mpu.hpp>
#include <cute/numeric/integer_sequence_mpu.hpp>
...
///
/// Throughout CUTLASS, maxl::make_tuple always needs to be called
/// namespace-qualified, EVEN If inside the cute namespace and/or in
/// scope of a "using namespace cute" declaration. Otherwise, the
/// compiler may select std::make_tuple instead of maxl::make_tuple,
...
// Shortcut around maxl::tuple_cat for common insert/remove/repeat cases
template <class T, class X, int... I, int... J, int... K>
CUTE_HOST_DEVICE constexpr
auto
construct(T const& t, X const& x, seq<I...>, seq<J...>, seq<K...>)
{
  return maxl::make_tuple(get<I>(t)..., (void(J),x)..., get<K>(t)...);
}
...
transform(T const& t, F&& f)
{
  if constexpr (is_tuple<T>::value) {
    return detail::tapply(t, f, [](auto const&... a){ return maxl::make_tuple(a...); }, tuple_seq<T>{});
  } else {
    return f(t);
  }
...
transform(T0 const& t0, T1 const& t1, F&& f)
{
  if constexpr (is_tuple<T0>::value) {
    ...
    return detail::tapply(t0, t1, f, [](auto const&... a){ return maxl::make_tuple(a...); }, tuple_seq<T0>{});
  } else {
    return f(t0, t1);
  }
...
transform(T0 const& t0, T1 const& t1, T2 const& t2, F&& f)
{
  if constexpr (is_tuple<T0>::value) {
    ...
    return detail::tapply(t0, t1, t2, f, [](auto const&... a){ return maxl::make_tuple(a...); }, tuple_seq<T0>{});
  } else {
    return f(t0, t1, t2);
  }
...
find_if(T const& t, F&& f)
{
  if constexpr (is_tuple<T>::value) {
    ...
    return maxl::C<decltype(f(t))::value ? 0 : 1>{};
  }
...
{
  if constexpr (is_tuple<T>::value) {
    return detail::apply(maxl::transform(t, f), [&] (auto const&... a) { return (false_type{} || ... || a); }, tuple_seq<T>{});
  } else {
    return f(t);
  }
...
{
  if constexpr (is_tuple<T>::value) {
    return detail::apply(maxl::transform(t, f), [&] (auto const&... a) { return (true_type{} && ... && a); }, tuple_seq<T>{});
  } else {
    return f(t);
  }
...
{
  return transform_apply(t, f, [](auto const&... a) { return maxl::tuple_cat(a...); });
}
...
{
  return transform_apply(t0, t1, f, [](auto const&... a) { return maxl::tuple_cat(a...); });
}
...
{
  return transform_apply(t0, t1, t2, f, [](auto const&... a) { return maxl::tuple_cat(a...); });
}
...
take(T const& t)
{
  ...
  if constexpr (B <= E) {
    return detail::apply(t, [](auto const&... a) { return maxl::make_tuple(a...); }, make_range<B,E>{});
  } else {
    static_assert(B <= E);
  }
...
select(T const& t)
{
  return maxl::make_tuple(get<I>(t)...);
}
...
unflatten_impl(FlatTuple const& flat_tuple, TargetProfile const& target_profile)
{
  if constexpr (is_tuple<TargetProfile>::value) {
    return fold(target_profile, maxl::make_tuple(maxl::make_tuple(), flat_tuple), [](auto const& v, auto const& t) {
      auto [result, remaining_tuple] = v;
      auto [sub_result, sub_tuple] = unflatten_impl(remaining_tuple, t);
      return maxl::make_tuple(append(result, sub_result), sub_tuple);
    });
  } else {
    return maxl::make_tuple(get<0>(flat_tuple), take<1, decltype(rank(flat_tuple))::value>(flat_tuple));
  }
...
append(T const& a, X const& x)
{
  if constexpr (is_tuple<T>::value) {
    ...
    if constexpr (N == 1) {
      ...
      return detail::construct(maxl::make_tuple(a), x, seq<0>{}, make_seq<N-1>{}, seq<>{});
    }
  }
...
prepend(T const& a, X const& x)
{
  if constexpr (is_tuple<T>::value) {
    ...
    if constexpr (N == 1) {
      ...
      static_assert(N > 1);
      return detail::construct(maxl::make_tuple(a), x, seq<>{}, make_seq<N-1>{}, seq<0>{});
    }
  }
...
zip_(Ts const&... ts)
{
  return maxl::make_tuple(get<J>(ts)...);
}
...
zip(T const& t, seq<Is...>, seq<Js...>)
{
  ...
  return maxl::make_tuple(zip_<Js>(get<Is>(t)...)...);
}
...
{
  return zip(maxl::make_tuple(t0, t1, ts...));
}
...
zip2_by(T const& t, TG const& guide, seq<Is...>, seq<Js...>)
{
  ...
  auto split = maxl::make_tuple(zip2_by(get<Is>(t), get<Is>(guide))...);

  // Rearrange and append missing modes from t to make ((A,B,...),(a,b,...,x,y))
  return maxl::make_tuple(maxl::make_tuple(get<0>(get<Is>(split))...),
                          maxl::make_tuple(get<1>(get<Is>(split))..., get<Js>(t)...));
}
...
{
  if constexpr (is_tuple<T>::value) {
    return detail::apply(t, [](auto const&... a){ return maxl::make_tuple(a...); }, tuple_rseq<T>{});
  } else {
    return t;
  }
}
