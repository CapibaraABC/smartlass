...
template <class ProblemShape, class CtaTiler,
          ...
          class TC, class SmemLayoutC, class CStride, class TiledMma,
            ...
            Alpha alpha, Beta beta)
{
  ...
  for (int pipe = 0; pipe < prologue_iterations; ++pipe)
  {
    if ((warp_idx == 0) && lane_predicate)
    {
      ...
      copy(dma_a.with(* tma_barrier), tAgA(_,0,0,k_tile), tAsA(_,pipe));
      copy(dma_b.with(* tma_barrier), tBgB(_,0,0,k_tile), tBsB(_,pipe));
  ...
  Tensor aurora_tCsA = thr_mma.aurora_partition_A_v2(sA);
  Tensor aurora_tCsB = thr_mma.aurora_partition_B_v2(sB);
  Tensor aurora_tCsC = thr_mma.aurora_partition_C_v3(sC);

  // Allocate accumulators and clear them
  Tensor aurora_tCrC = thr_mma.make_fragment_C_new(aurora_tCsC);                        // (MMA,MMA_M,MMA_N)
  clear(aurora_tCrC);

  // // Allocate "fragments"
  Tensor aurora_tCrA = thr_mma.make_fragment_A(aurora_tCsA);                         // (MMA,MMA_M,MMA_K,PIPE)
  Tensor aurora_tCrB = thr_mma.make_fragment_B(aurora_tCsB);                         // (MMA,MMA_N,MMA_K,PIPE)
  // if (thread0()) {
  ...
  while (cal_loop !=  size<3>(tAgA))
  {
    ...
    gemm(mma, aurora_tCrA(_,_,_,read_pipe), aurora_tCrB(_,_,_,read_pipe), aurora_tCrC(_,_,_,read_pipe));
        ...
        cudaStream_t stream = 0)
{
  ...
  void const* kernel_ptr = reinterpret_cast<void const*>(
                              &gemm_device<decltype(prob_shape), decltype(cta_tiler),
                                           ...
                                           TC, decltype(sC), decltype(dC), decltype(tiled_mma),
