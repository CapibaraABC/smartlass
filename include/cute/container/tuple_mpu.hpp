...
#include <cute/numeric/integral_constant_mpu.hpp>  // cute::true_type, cute::false_type
#include <cute/numeric/integer_sequence_mpu.hpp>

#include <cute/container/cuda_types.hpp>
#include <cute/container/type_list_mpu.hpp>
...
static constexpr bool is_first_empty_v = maxl::is_empty<First>::value;
template <class First, class... Rest>
static constexpr bool is_rest_empty_v  = (maxl::is_empty<Rest>::value && ...);
...
CUTE_HOST_DEVICE constexpr
maxl::enable_if_t<maxl::is_empty<maxl::tuple_element_t<N, maxl::type_list<T, Rest...>>>::value,
                                 maxl::tuple_element_t<N, maxl::type_list<T, Rest...>>>
...
template <size_t N, bool F, bool R, class T, class... Rest>
CUTE_HOST_DEVICE constexpr
maxl::enable_if_t<not maxl::is_empty<maxl::tuple_element_t<N, maxl::type_list<T, Rest...>>>::value,
                                     maxl::tuple_element_t<N, maxl::type_list<T, Rest...>> const&>
...
{
  if constexpr (maxl::is_same_v<X, First>) {
    return C<N>{};
  } else
  ...
  if constexpr (IsRestEmpty) {
    return maxl::detail::findt<X, N+1>(ESO_t<Rest...>{});
  } else {
    return maxl::detail::findt<X, N+1>(t.rest_);
  }
}
...
template <class T>
constexpr bool is_tuple_v = maxl::is_tuple<T>::value;
...
template <class... T>
struct tuple_size<maxl::tuple<T...>>
    : MAXL_STL_NS::integral_constant<size_t, sizeof...(T)>
{};

template <size_t I, class... T>
struct tuple_element<I, maxl::tuple<T...>>
    : MAXL_STL_NS::tuple_element<I, MAXL_STL_NS::tuple<T...>>
{};

} // end namespace MAXL_STL_NS
