...
#include <cute/layout_mpu.hpp>                     // cute::Shape
...
#include <cute/container/array_aligned_mpu.hpp>    // cute::array_aligned
#include <cute/container/array_subbyte.hpp>    // cute::array_subbyte
#include <cute/container/tuple_mpu.hpp>            // cute::tuple
#include <cute/numeric/integral_constant_mpu.hpp>  // cute::is_integral
#include <cute/util/type_traits_mpu.hpp>           // __MAXL_REQUIRES
...
struct Tensor
{
  ...
  template <class Int,
            __MAXL_REQUIRES(is_integral<Int>::value)>
  CUTE_HOST_DEVICE constexpr
  auto
  ...
  maxl::tuple<layout_type, engine_type> rep_;
};
...
template <class Layout, __MAXL_REQUIRES(is_layout<Layout>::value)>
CUTE_HOST_DEVICE constexpr
auto
...
template <class Coord, class Tensor,
          __MAXL_REQUIRES(is_tensor<remove_cvref_t<Tensor>>::value)>
CUTE_HOST_DEVICE constexpr
auto
...
recast(Tensor&& tensor)
{
  ...
  if constexpr (sizeof(OldType) < sizeof(NewType) && not is_composed_layout<decltype(old_layout)>::value) {
    ...
    auto offset = fold(extent_diff, Int<0>{}, [](auto const& i, auto const& a) { return i + maxl::min(a,Int<0>{}); });
                  ...
                  Tensor<DstEngine,DstLayout> const& b)
{
  ...
  if constexpr (// Should be the same value_types, else the copy is also performing a cast
                maxl::is_same<SrcType, DstType>::value &&
