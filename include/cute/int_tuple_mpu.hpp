...
#include <cute/numeric/integral_constant_mpu.hpp>   // cute::Int
#include <cute/algorithm/tuple_algorithms_mpu.hpp>  // cute::transform
...
namespace maxl
{
template <size_t I, class T, __MAXL_REQUIRES(maxl::is_integral<maxl::remove_cvref_t<T>>::value)>
  ...
  operator()(IntTuple const& a) const
  {
    if constexpr (is_tuple<IntTuple>::value) {
      if constexpr (tuple_size<IntTuple>::value == 0) {
        ...
        return maxl::transform_apply(a, Product{}, maxl::multiplies_unary_lfold{});
      }
    } else if constexpr (maxl::is_integral<IntTuple>::value) {
      return a;
    }
...
MAXL_INLINE_CONSTANT Product product;
...
size(IntTuple const& a)
{
  if constexpr (sizeof...(Is) == 0) {
    ...
    return maxl::size(maxl::get<Is...>(a));
  }
