...
#include <cute/config_mpu.hpp>                     // CUTE_HOST_DEVICE
#include <cute/util/type_traits_mpu.hpp>           // cute::__CUTE_REQUIRES
#include <cute/container/tuple_mpu.hpp>            // cute::is_tuple
#include <cute/numeric/integral_constant_mpu.hpp>  // cute::is_integral
#include <cute/numeric/integer_sequence_mpu.hpp>   // cute::seq
#include <cute/numeric/math.hpp>               // cute::divmod
#include <cute/numeric/arithmetic_tuple_mpu.hpp>   // cute::basis_get
#include <cute/algorithm/functional_mpu.hpp>       // cute::identity
#include <cute/algorithm/tuple_algorithms_mpu.hpp> // cute::fold
#include <cute/int_tuple_mpu.hpp>                  // cute::is_congruent
        ...
        Current const& current)
{
  if constexpr (is_tuple<Shape>::value) { // Shape::tuple Current::int
    ...
    return maxl::detail::fold(shape, maxl::make_tuple(maxl::make_tuple(), current), Lambda{}, Seq{});
  } else {                                // Shape::int Current::int
    if constexpr (is_constant<1, Shape>::value) {
      return maxl::make_tuple(Int<0>{}, current); // If current is dynamic, this could save a reg
    } else {
      return maxl::make_tuple(current, current * shape);
    }
  }
...
struct CompactLambda<LayoutLeft>
{
  ...
  operator()(Init const& init, Shape const& si) {
    ...
    return maxl::make_tuple(append(get<0>(init), get<0>(result)), get<1>(result));  // Append
  }
...
struct CompactLambda<LayoutRight>
{
  ...
  operator()(Init const& init, Shape const& si) {
    ...
    return maxl::make_tuple(prepend(get<0>(init), get<0>(result)), get<1>(result));  // Prepend
  }
...
template <class Major, class Shape, class Current = Int<1>,
          __MAXL_REQUIRES(is_tuple<Shape>::value || is_integral<Shape>::value)>
CUTE_HOST_DEVICE constexpr
auto
