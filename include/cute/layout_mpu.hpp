...
#include <cute/config_mpu.hpp>
#include <cute/int_tuple_mpu.hpp>
#include <cute/stride_mpu.hpp>
#include <cute/underscore_mpu.hpp>
#include <cute/numeric/arithmetic_tuple_mpu.hpp>
...
struct Layout
    : private maxl::tuple<Shape, Stride>   // EBO for static layouts
{
  ...
  Layout(Shape  const& shape  = {}, Stride const& stride = {})
      : maxl::tuple<Shape, Stride>(shape, stride)
  {}
  ...
  decltype(auto)
  shape() {
    return get<0,I...>(static_cast<maxl::tuple<Shape, Stride>&>(*this));
  }
  ...
  decltype(auto)
  shape() const {
    return get<0,I...>(static_cast<maxl::tuple<Shape, Stride> const&>(*this));
  }
  ...
  decltype(auto)
  stride() {
    return get<1,I...>(static_cast<maxl::tuple<Shape, Stride>&>(*this));
  }
  ...
  decltype(auto)
  stride() const {
    return get<1,I...>(static_cast<maxl::tuple<Shape, Stride> const&>(*this));
  }
  ...
  get_hier_coord(IInt const& idx) const {
    return maxl::idx2crd(idx, shape(), stride());
  }
  ...
  get_flat_coord(IInt const& idx) const {
    return maxl::crd2crd(this->get_hier_coord(idx), shape(), repeat<rank>(Int<1>{}));
  }
  ...
  get_1d_coord(IInt const& idx) const {
    return maxl::crd2idx(this->get_hier_coord(idx), shape());
  }
  ...
  crd_2_hier_coord(Coord const& crd) const {
    return maxl::crd2crd(crd, shape(), shape());
  }
  ...
  crd_2_flat_coord(Coord const& crd) const {
    return maxl::crd2crd(crd, shape(), product_each(shape()));
  }
...
template <class... Shapes>
using Shape = maxl::tuple<Shapes...>;

template <class... Strides>
using Stride = maxl::tuple<Strides...>;

template <class... Strides>
using Step = maxl::tuple<Strides...>;

template <class... Coords>
using Coord = maxl::tuple<Coords...>;

template <class... Layouts>
using Tile = maxl::tuple<Layouts...>;
