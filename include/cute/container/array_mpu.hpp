...
/// @return A maxl::array of the elements of @c t in reverse order.
template <class T, size_t N>
CUTE_HOST_DEVICE constexpr
maxl::array<T,N> reverse(maxl::array<T,N> const& t)
{
  if constexpr (N == 0u) {
    return t;
  } else {
...
//
// Specialize tuple-related functionality for maxl::array
//
...
template <class T, size_t N>
struct tuple_size<maxl::array<T,N>>
    : CUTE_STL_NAMESPACE::integral_constant<size_t, N>
{};

template <size_t I, class T, size_t N>
struct tuple_element<I, maxl::array<T,N>>
{
  using type = T;
};
